...
int main(int argc, char **argv) {
  ...
  const std::string node_name = "lidar_odometry_" + random_string(10);
  ...
  CLOG(WARNING, "test") << "Publishing status to topic: " << (stem + "_lidar_odometry");
  const auto status_publisher = node->create_publisher<std_msgs::msg::String>(stem + "_lidar_odometry", 1);
