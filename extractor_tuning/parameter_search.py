...
# USER INPUT: Specify if you want all the logs, or just the basic ones
dense_logging = False
...
def read_last_lines(filepath, num_lines=100):
    ...
    Args:
        filepath (str): The path to the log file to read.
        num_lines (int, optional): The number of lines to read from the end of the file. Defaults to 100.
    Returns:
        str: A string containing the last `num_lines` lines of the file.
    Raises:
...
def poll_extracted_point_count(odom_folder, sequence):
    """
    Polls the extracted point count from the most recent log file in the specified odometry folder.
    Args:
        odom_folder (str): The folder containing odometry logs of the specific extractor-sequence-parameter instance being run.
        sequence (str): The sequence name.
    Returns:
        bool: True if no log files are found or if the extracted point count exceeds the maximum allowed points, False otherwise.
    """
...
def run_test(config_data, MODE, SENSOR, SEQUENCE, config_type, val_1=None, val_2=None, val_3=None):
    """
    Executes a single test run for radar topometric localization based on the provided parameters and sequence.
    Args:
        config_data (dict): Configuration data to be used in the yaml file.
        MODE (str): Mode of operation (Odometry).
        SENSOR (str): Sensor type (Always radar).
        SEQUENCE (str): Sequence name.
        config_type (str): Extractor name.
        val_1 (optional): Additional parameter value 1.
        val_2 (optional): Additional parameter value 2.
        val_3 (optional): Additional parameter value 3.
    Returns:
        tuple: A tuple containing the process ID of the current process running in the background 
        and the ODO_INPUT string specifying the directory this process is writing to.
    """
    ...
    # Create extractor-sequence specific folder name 
    # (including extractor name, sequence, and relevent parameters)
    ...
    if not just_eval:
        try:
            ...
            print(f"Failed to create new directory '{destination}': {error}")

    # Create new config file and load the settings
    ...
    # Command for vtr_testing_radar script
    ...
    if curr_process_count >= max_process_count and not just_eval:
        ...
        print("Waiting; Number of processes running:",curr_process_count, " Number of finished process:",finished_process_count, " total count: ", total_process_count)
        while curr_head_proc.poll() is None:
            # Poll the current sequence to check the max_points condition is being violated
            halt_proc = poll_extracted_point_count(curr_odo_input, SEQUENCE)
            if halt_proc:
                time.sleep(0.5)
    ...
    if not just_eval: 
        # Run setup and odometry scripts
    ...
    # If we just evaluate, assign dummy process number
    else: process = 1
...
def run_eval(processes, result_dirs, SENSOR, seq, config):
    """
    Evaluates the results of all the odometry instances for the extractor-sequence pair
    Parameters:
    processes (list): List of subprocess.Popen objects representing the running processes.
    result_dirs (list): List of directories where the results of each process are stored.
    SENSOR (str): The type of sensor being used (Always radar).
    seq (str): The name of the sequence being evaluated.
    config (str): The detector name.

    The function performs the following steps:
    1. Ensures all processes have finished execution before proceeding to evaluation.
    2. Sets up the results directory for each process.
    3. Generates odometry results using the boreas_generate_odometry_result module.
    4. Evaluates the odometry results and appends the metrics to a results list.
    5. Writes the evaluation metrics to a CSV file in the results directory.
    Returns:
    None
    """
    ...
    for count, process in enumerate(processes):
        ...
        # Check again that all the processes have finished before evaluation
        if not just_eval and process.poll() is None:
            print("Waiting; Number of processes running:",curr_process_count, " Number of finished process:",finished_process_count)
            while process.poll() is None:
                # Poll the current sequence to check the max_points condition is being violated
                halt_proc = poll_extracted_point_count(result_dir,seq)
                if halt_proc:
                    time.sleep(0.5)
                    # Kill the entire process group
        ...
        try:
            # Expecting that eval_odom will return these four metrics
            curr_t_err, curr_r_err = eval_odom(pred=pred,gt=VTRRDATA,radar=True)
            results.append([result_dir, curr_t_err, curr_r_err])
            if eval_vel:
            ...
            results.append([result_dir, 0.0, 0.0])
    ...
    with open(csv_file, 'w', newline='') as file_ref:
        ...
        for result_dir, curr_t_err, curr_r_err in results:
            file.writerow([result_dir, float(curr_t_err), float(curr_r_err)])
...
if __name__ == "__main__":
    ...
    for count, parameter in enumerate(config_settings["parameters"]):
        parameter_names[count] = parameter
    ...
    parameter_1_range = parameter_1_range[parameter_1_range <= parameter_1["max"]]
    ...
    for val_1 in parameter_1_range:
        ...
        parameter_2_range = parameter_2_range[parameter_2_range <= parameter_2["max"]]
